...
//                 wrapper.getElementsByClassName("fp-section-container")[0]
//               ).transitionDuration
//             : comp.transitionDuration;
...
//       }
//       doc
//         .tfOn("keydown", _keydown, { passive: true })
//         .tfOn("wheel", _wheel, { passive: true });
//       if (Themify.isTouch) {
//         wrapper.tfOn(
//           win.PointerEvent ? "pointerdown" : "touchstart",
//           _touchstart,
//           { passive: true }
//         );
//       }
...
//       isDisabled = true;
//       doc
//         .tfOff("keydown", _keydown, { passive: true })
...
//         node.classList.contains("fp-section-container") ||
//         node.classList.contains("tf_scrollbar")
//       ) {
//         return false;
//       }
...
//             let vHeight =
//                 i === 0 || fixedHeight === 0 ? normalHeight : fixedHeight,
//               row_inner,
...
//                     wrap.className =
//                       "fp-section-container tf_w tf_h tf_overflow";
//                     if (paddings[0] !== "") {
...
//               }
//               container.className =
//                 "fp-section-container-horizontal tf_w tf_rel tf_overflow";
...
//       if (initOnce === false) {
//         for (
//           let allLazy = doc.querySelectorAll("[data-lazy]"),
//             i = allLazy.length - 1;
//           i > -1;
//           --i
//         ) {
...
//         for (
//           let wowItems = wrapper.tfClass("wow"), i = wowItems.length - 1;
//           i > -1;
//           --i
//         ) {
...
//             let sectionEl =
//               wrapper.querySelector('[data-anchor="' + anchor + '"]') ||
//               doc.tfId(anchor);
//             if (sectionEl !== null) {
//               sectionEl = sectionEl.closest(".fp-section-container");
//               if (sectionEl) {
//                 let verticalIndex = Themify.convert(
//                     sectionEl.parentNode.children
//                   ).indexOf(sectionEl),
//                   horizontalIndex = und;
//                 const horizontal = sectionEl.closest(
//                   ".fp-section-container-horizontal"
//                 );
//                 if (horizontal) {
//                   horizontalIndex = verticalIndex;
//                   verticalIndex = Themify.convert(
//                     horizontal.parentNode.children
//                   ).indexOf(horizontal);
//                 }
...
//           //should be click,browser break in mobile,after clicking to the link that target row id
...
//                         path.hostname === location.hostname))
//                   ) {
//                     e.preventDefault();
...
//           scrollTo(
//             currentIndex,
//             Themify.convert(el.parentNode.children).indexOf(el)
//           );
//           isLargeScrolling = false;
//         }
...
//       if (
//         scrolling === false &&
//         isOverflow(e.target) === false &&
//         !(Themify.isTouch && e.target.closest(".themify_builder_slider"))
//       ) {
...
//             this.tfOff(_MOVE_, _moveCallback, { passive: true }).tfOff(
//               [_UP_, _CANCEL_],
//               _upCallback,
...
//                 (inHorizontal === true && touchEndX !== touchStartX)
//               ) {
//                 let dir = "";
//                 if (inHorizontal === true) {
//                   if (touchEndX + _SENSITIVE_ < touchStartX) {
//                     /*left*/
//                     dir = Themify.isRTL === true ? "swipe_prev" : "swipe_next";
//                   } else if (touchEndX - _SENSITIVE_ > touchStartX) {
//                     /*right*/
//                     dir = Themify.isRTL === true ? "swipe_next" : "swipe_prev";
//                   }
//                 }
//                 if (dir === "") {
//                   if (touchEndY + _SENSITIVE_ < touchStartY) {
//                     /*up*/
//                     dir = "next";
//                   } else if (touchEndY - _SENSITIVE_ > touchStartY) {
//                     /*down*/
//                     dir = "prev";
//                   }
//                 }
...
//             inHorizontal =
//               target.closest(".fp-section-container-horizontal") !== null;
//           }
//           doc
//             .tfOn(_MOVE_, _moveCallback, { passive: true })
...
//             el &&
//             _ISPARALLAX_ === true
//               ? el.tfClass("module_row")[0]
//               : null,
//           nav = doc.tfId("fp-nav"),
//           ev =
//             currentIndex === 0
//               ? "tf_fixed_header_disable"
//               : "tf_fixed_header_enable";
//         if (row) {
...
//                   e.target.parentNode.parentNode.parentNode === this))
//             ) {
//               this.tfOff("transitionend", trEnd, { passive: true });
//               el.classList.add("complete");
//               const __callback = () => {
...
//             if (
//               e.target === this ||
//               (isFade === true && e.target.parentNode === this)
//             ) {
//               this.tfOff("transitionend", trEnd, { passive: true });
//               el.classList.add("complete");
//               const __callback = () => {
...
//           "translateX(" + (Themify.isRTL ? "" : "-") + 100 * index + "%)";
//         Themify.trigger("themify_onepage_afterload", [el]);
//       }
//     },
...
//         let keyDown =
//           typeof verticalIndex === "string" &&
//           verticalIndex.indexOf("Key") > -1;
//         if (keyDown) {
//           keyDown = verticalIndex;
//           verticalIndex =
//             verticalIndex === "nextKey" || verticalIndex === "rightKey"
//               ? "next"
//               : "prev";
//         }
//         // Detect Touch swipe
...
//             isHorizontalScroll =
//               isHorizontal === true ||
//               (item.classList.contains("fp-section-container-horizontal")
//                 ? isNumber ||
//                   snakeScroll ||
//                   swipe !== false ||
//                   keyDown === "leftKey" ||
//                   keyDown === "rightKey"
//                 : false);
//           const horizontalChilds = isHorizontalScroll
//               ? item.tfClass("fp-section-container")
//               : null,
//             horizontalItem =
//               isHorizontalScroll && horizontalChilds[index]
//                 ? horizontalChilds[index]
//                 : null;
//           if (
//             isHorizontalScroll &&
//             isNumber &&
//             (!horizontalChilds[horizontalIndex] ||
//               !item.contains(horizontalChilds[horizontalIndex]))
//           ) {
...
//               if (
//                 (verticalIndex === "prev" && top > 0) ||
//                 (verticalIndex === "next" && top < max - 3)
//               ) {
...
//             } else if (
//               horizontalChilds[index] &&
//               horizontalChilds[index].nextElementSibling
//             ) {
...
//             if (
//               !isNumber &&
//               verticalChilds[currentIndex] &&
//               verticalChilds[currentIndex].classList.contains(
//                 "fp-section-container-horizontal"
//               )
//             ) {
//               const index =
...
//                   ? 0
//                   : snakeScroll
//                   ? verticalChilds[currentIndex].tfClass("fp-section-container")
//                       .length - 1
//                   : 0;
//               if (
//                 index !==
//                 parseInt(verticalChilds[currentIndex].dataset["index"])
//               ) {
...
//           } else if (
//             verticalChilds[currentIndex] &&
//             verticalChilds[currentIndex].nextElementSibling
//           ) {
...
//       const active = wrapper.querySelectorAll(
//           ".fp-section-container-horizontal.active,.fp-section-container.active"
//         ),
//         verticalIndex = currentIndex,
//         verticalItem = wrapper.children[verticalIndex],
//         isHorizontalScroll = horizontalIndex === und,
//         isHorizontalWrapper = verticalItem.classList.contains(
//           "fp-section-container-horizontal"
//         ),
//         bodyCl = doc.body.classList;

//       let activeCl =
//           isHorizontal === true || isHorizontalWrapper
//             ? verticalIndex
//             : _getAnchor(verticalItem.tfClass("module_row")[0], true),
...
//         }
//         currentSection = verticalItem.tfClass("fp-section-container")[
//           horizontalIndex
//         ];
...
//         if (
//           childs[j].classList.contains("module_row") &&
//           (childs[j].classList.contains("hide-" + bp) ||
//             (childs[j].offsetWidth === 0 && childs[j].offsetHeight === 0))
//         ) {
//           childs[j].remove();
//         }
//       }
...
//         Themify.lazyScroll(
//           Themify.convert(
//             Themify.selectWithParent("[data-lazy]", el)
//           ).reverse(),
//           true
//         );
//       }
...
//         let aSectionHref = anchor
//           ? menu.querySelector('a[href="#' + anchor.replace("#", "") + '"]')
//           : null;
...
//               if (item) {
//                 //if there is an element,browser will move the scrollbar
...
//             if (
//               isMobile === true &&
//               bodyCl.contains("full-section-scrolling")
//             ) {
//               _disable();
...
//                 for (
//                   let childs = wrapper.children, i = childs.length - 1;
//                   i > -1;
//                   --i
//                 ) {
//                   let row_inner = childs[i].tfClass("row_inner")[0];
//                   if (row_inner !== und) {
//                     let paddings = _getPaddings(
//                       childs[i].tfClass("module_row")[0],
//                       bp
//                     );
//                     if (paddings[0] !== "") {
//                       row_inner.style.paddingTop = paddings[0];
...
//         }
//         Themify.on(
//           "themify_builder_loaded",
//           callback,
//           true,
//           Themify.is_builder_loaded || win.tbLocalScript === und
//         );
//       } else {
//         Themify.trigger("themify_onepage_afterload");
//       }
...
//           isNotTransparent =
//             !bodyCl.contains("transparent-header") &&
//             !bodyCl.contains("menubar-bottom") &&
//             !bodyCl.contains("menubar-top");
//         let offset = 0;
...
//               },
//               true,
//               typeof ThemifyFixedHeader !== "undefined" &&
//                 typeof ThemifyFixedHeader.headerWrap !== null
//             );
//           } else {
...
//       wrapper =
//         wrapper !== null
//           ? wrapper.tfClass("themify_builder")[0]
//           : doc.querySelector(".themify_builder:not(.not_editable_builder)");
//       Themify.wow().then(() => {
